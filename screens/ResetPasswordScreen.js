  ...
  return (
    <View style={styles.container}>
      ...
      <View style={{margin: 10}}>
        <Text style={{fontSize: textFontSize, textAlign: 'center'}}>Enter your email, and we’ll send you instructions to reset your password.</Text>
      </View>
    ...
    <Text style={{ fontSize: emailFontSize, alignSelf: 'flex-start', marginLeft: marginLeftTest, marginBottom: 10, marginTop: 10 }}>Email</Text>
      ...
      >
        <Text style={{ fontSize: emailFontSize, color: 'white', textAlign: 'center', fontWeight: 'bold' }}>Send Reset Link</Text>
      </TouchableOpacity>
      {errorMessage ? <Text style={styles.errorMessage}>{errorMessage}</Text> : null}
      {isResetSuccessful ? (
        <Text style={styles.successMessage}>
        A password reset link has been sent to your email.</Text>
      ) : null}
