...
const ReportScreen = () => {
  ...
  const formatPatientsData = (data) => {
    return data.map(patient => {
      return {
        ...patient,
        coMorbid: patient.coMorbid ? patient.coMorbid.map(diagnosis => diagnosis.value).join(', ') : '',
        ...
        approvalTimestamp: patient.approvalTimestamp ? new Date(patient.approvalTimestamp.seconds * 1000).toLocaleDateString() : '', // แปลง timestamp เป็นวันที่
        rejectionTimestamp: patient.rejectionTimestamp ? new Date(patient.rejectionTimestamp.seconds * 1000).toLocaleDateString() : '', // แปลง timestamp เป็นวันที่
      };
    });
  };
  ...
  const formatDataForExport = (data) => {
    return data.map(procedure => {
      return {
        ...procedure,
        admissionDate: procedure.admissionDate ? new Date(procedure.admissionDate.seconds * 1000).toLocaleDateString() : '', // แปลง timestamp เป็นวันที่
        approvalTimestamp: procedure.approvalTimestamp ? new Date(procedure.approvalTimestamp.seconds * 1000).toLocaleDateString() : '', // แปลง timestamp เป็นวันที่
        rejectionTimestamp: procedure.rejectionTimestamp ? new Date(procedure.rejectionTimestamp.seconds * 1000).toLocaleDateString() : '', // แปลง timestamp เป็นวันที่
  ...
  const handleDownload = () => {
    ...
    const sortedProceduresData = formatDataForExport(proceduresData).sort((a, b) => {
      const dateA = new Date(a.admissionDate);
      const dateB = new Date(b.admissionDate);
      return dateA - dateB;
    });
    ...
    const translateProfessionalismScores = (scores) => {
      ...
      if (scores) {
        return Object.entries(scores).filter(([key, value]) => value).map(([key]) => translationMap[key] || key).join(', ');
      } else {
        return '';
      }
    };
  ...
  const formattedPatientsData = sortedPatientsData.map(patient => ({
    ...
    'คะแนนการประเมิน (professionalismScores)': translateProfessionalismScores(patient.professionalismScores)
  }));
