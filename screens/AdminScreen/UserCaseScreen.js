...
import { collection, getDocs, doc, getDoc, setDoc, updateDoc, Timestamp, deleteDoc, where, query} from "firebase/firestore";
...
function UserCaseScreen({navigation}) {
  ...
  const handleChange = async () => {
    try {
      ...
      if (selectedCase && selectedCase.id) { // ตรวจสอบว่า selectedCase ไม่เป็น null และมี id
  ...
  const renderCards = () => {
    const allCases = filterCasesByType(selectedType).filter(caseData => caseData.status === selectedStatus);
        ...
        >
          <View style={styles.card}>
            <View style={styles.leftContainer}>
              {caseData.procedureType || caseData.activityType ? (
                <>
                  ...
                  <Text style={{ marginLeft: 20, lineHeight: 30, opacity: 0.4 }}>
                    {caseData.procedureType ? 'Approved By' : 'Professor Name'}: {caseData.procedureType ? caseData.approvedByName : caseData.professorName}
                  </Text>
                </>
              ) : (
                <>
              ...
              <Text style={{ marginLeft: 20, lineHeight: 30, opacity: 0.4 }}>
                <FontAwesome name="calendar" size={20} color="black" /> {formatDate2(caseData.admissionDate.toDate())} | {formatTimeUnit(caseData.hours)}.{formatTimeUnit(caseData.minutes)}
              </Text>
            ) : (
              <Text style={{ marginLeft: 20, lineHeight: 30, opacity: 0.4 }}>
                <FontAwesome name="calendar" size={20} color="black" />
                {" "} {formatDate2(caseData.admissionDate.toDate())} | {formatTimeUnit(caseData.hours)}.{formatTimeUnit(caseData.minutes)}
                {caseData.approvalTimestamp && (
                  <Text> (Approved: {formatDateToThai(caseData.approvalTimestamp.toDate())})</Text>
                )}
                {caseData.rejectionTimestamp && (
                  <Text> (Rejected: {formatDateToThai(caseData.rejectionTimestamp.toDate())})</Text>
                )}
              </Text>
            )}
            </View>
  ...
  const filterCasesByType = (type) => {
    switch (type) {
      case 'inpatient':
        return patientData.filter(caseData => caseData.status === selectedStatus);
      case 'outpatient':
        return outpatientData.filter(caseData => caseData.status === selectedStatus);
      case 'procedure':
        return procedureData.filter(caseData => caseData.status === selectedStatus);
      case 'activity':
        return activityData.filter(caseData => caseData.status === selectedStatus);
      default:
        return [
          ...patientData.filter(caseData => caseData.status === selectedStatus),
          ...outpatientData.filter(caseData => caseData.status === selectedStatus),
          ...procedureData.filter(caseData => caseData.status === selectedStatus),
          ...activityData.filter(caseData => caseData.status === selectedStatus)
        ];
    }
  };
        ...
        <View style={styles.container}> 
          ...
          <View style={{ marginVertical: 10, flexDirection: 'row', alignItems: 'center' }}>
            ...
            <SelectList
              ...
              search={false}
              boxStyles={{ width: 'auto', backgroundColor: '#FEF0E6', borderColor: '#FEF0E6', borderWidth: 1, borderRadius: 10, marginLeft: 15 }}
              dropdownStyles={{ backgroundColor: '#FEF0E6' }}
            />
          </View>
          ...
          >
            <View style={styles.centerView}>
              <View style={styles.modalView}>
                <Text style={styles.modalText}>Change status this case to pending?</Text>
                      ...
                      {selectedCase.hours !== '' && selectedCase.minutes !== '' && (
                        <>
                          <Text style={styles.modalText}>
                            ...
                            {formatTimeUnit(selectedCase.hours)}.{formatTimeUnit(selectedCase.minutes)}
                          </Text>
                        </>
                      )}
                      <Text style={styles.modalText}>
                        <Text style={{ fontWeight: "bold" }}>{selectedCase.procedureType ? 'Approved By' : 'Professor Name'}: </Text> {selectedCase.procedureType ? selectedCase.approvedByName : selectedCase.professorName}
                      </Text>
                      {selectedCase.activityType && (
                        <Text style={styles.modalText}>
                          <Text style={{ fontWeight: "bold" }}>Type :</Text> {selectedCase.activityType}
                        </Text>
                      )}
                      {selectedCase.procedureType && (
                        <Text style={styles.modalText}>
                          <Text style={{ fontWeight: "bold" }}>Type :</Text> {selectedCase.procedureType}
                        </Text>
                      )}
                      {selectedCase.hn && (
                        <Text style={styles.modalText}>
                          <Text style={{ fontWeight: "bold" }}>HN :</Text> {selectedCase.hn}
                        </Text>
                      )}
                      {selectedCase.procedureLevel && (
                        <Text style={styles.modalText}>
                          <Text style={{ fontWeight: "bold" }}>Level :</Text> {displayLevel(selectedCase.procedureLevel)}
                        </Text>
                      )}
                      {selectedCase.mainDiagnosis && (
                        <Text style={styles.modalText}>
                          <Text style={{ fontWeight: "bold" }}>Main Diagnosis :</Text> {selectedCase.mainDiagnosis}
                        </Text>
                      )}
                      {selectedCase.coMorbid && (
                        <Text style={styles.modalText}>
                          <Text style={{ fontWeight: "bold" }}>Co - Morbid Diseases : </Text>
                          {selectedCase.coMorbid && selectedCase.coMorbid.length > 0 && selectedCase.coMorbid.some(diagnosis => diagnosis.value)
                            ? selectedCase.coMorbid.map(diagnosis => diagnosis.value).join(', ')
                            : "ไม่ระบุ"}
                        </Text>
                      )}
                      <Text style={styles.modalText}>
                        <Text style={{ fontWeight: "bold" }}>Note/Reflection : </Text> {selectedCase.note || "ไม่มี"}
                      </Text>
                      {selectedCase.rating && (
                        <Text style={styles.modalText}>
                          <Text style={{ fontWeight: "bold" }}>Rating :</Text> {selectedCase.rating}
                        </Text>
                      )}
                      {selectedCase.comment && (
                        <Text style={styles.modalText}>
                          <Text style={{ fontWeight: "bold" }}>***Comment :</Text> {selectedCase.comment}
                        </Text>
                      )}
                  ...
                  {selectedCase.professionalismScores && (
                    <>
                      ...
                      <Text style={styles.modalText}>
                        ...
                        {selectedCase.professionalismScores.appropriatelyDressed ? '✔️' : '❌'}
                      </Text>
                      <Text style={styles.modalText}>
                        <Text style={{ fontWeight: "bold", fontSize: 20 }}>Respect the Case : </Text>
                        {selectedCase.professionalismScores.respectsCases ? '✔️' : '❌'}
                      </Text>
                      <Text style={styles.modalText}>
                        <Text style={{ fontWeight: "bold", fontSize: 20 }}>Good listener : </Text>
                        {selectedCase.professionalismScores.goodListener ? '✔️' : '❌'}
                      </Text>
                      <Text style={styles.modalText}>
                        <Text style={{ fontWeight: "bold", fontSize: 20 }}>Respect colleagues : </Text>
                        {selectedCase.professionalismScores.respectsColleagues ? '✔️' : '❌'}
                      </Text>
                      <Text style={styles.modalText}>
                        <Text style={{ fontWeight: "bold", fontSize: 20 }}>Accurately record Case information : </Text>
                        {selectedCase.professionalismScores.accurateRecordKeeping ? '✔️' : '❌'}
                      </Text>
                    </>
                  )}
          ...
          >
            <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(0, 0, 0, 0.5)' }}>
              <View style={{ width: '70%', height: '70%', backgroundColor: 'white', borderRadius: 10 }}>
                <ScrollView>
                  {selectedCase?.images && selectedCase.images.map((imageUrl, index) => {
                    return (
