...
const ExportScreen = () => {
  ...
  const getSelectedDataForExport = () => {
    ...
    if (selectedData === 'patients') {
      dataForExport = patientsData.filter(patient => 
        (selectedStatus === 'all' || patient.status === selectedStatus) &&
        (selectedType === 'all' || patient.patientType === selectedType)
      );
    } else {
      dataForExport = proceduresData.filter(procedure => selectedStatus === 'all' || procedure.status === selectedStatus);
    }
  ...
  const handleDownload = () => {
    ...
    const sortedData = selectedData === 'patients' ? sortPatientsByAdmissionDate(dataForExport) : sortProceduresByAdmissionDate(dataForExport);
    const formattedData = selectedData === 'patients' ? formatPatientsDataForExport(sortedData) : formatProceduresDataForExport(sortedData);

    const ws = XLSX.utils.json_to_sheet(formattedData);
    XLSX.utils.book_append_sheet(wb, ws, `${selectedData.charAt(0).toUpperCase() + selectedData.slice(1)} Data`);
    ...
    if (fileFormat === 'csv') {
      ...
      csvLink.setAttribute('download', `${selectedData.charAt(0).toUpperCase() + selectedData.slice(1)}Data.csv`);
      csvLink.click();
    } else {
      XLSX.writeFile(wb, `${selectedData.charAt(0).toUpperCase() + selectedData.slice(1)}Data.${fileFormat}`);
    }
  };
  ...
  const translateProfessionalismScores = (scores) => {
    if (scores) {
      ...
      return JSON.stringify(Object.entries(scores).filter(([key, value]) => value).map(([key]) => translationMap[key] || key).join(', '));
    } else {
      return '';
    }
  };
  ...
  const formatPatientsDataForExport = (data) => {
    return data.map(patient => ({
      ...
      'Co-Morbid': patient.coMorbid ? patient.coMorbid.map(item => item.value).join(', ') : '', // แปลง JSON ให้เป็นสตริง
      ...
      'Professionalism Scores': translateProfessionalismScores(patient.professionalismScores || null),
    }));
  };
  ...
  const formatProceduresDataForExport = (data) => {
    return data.map(procedure => ({
      ...
      'Approval Timestamp': formatTimestamp(procedure.approvalTimestamp || null),
      'Rejection Timestamp': formatTimestamp(procedure.rejectionTimestamp || null),        
            ...
            <View style={styles.table}>
              ...
              {selectedData === 'patients' ? (
                <>
                  ...
                  <Text style={styles.columnHeader}>Professionalism Scores</Text>
                </>
              ) : (
                <>
                  <Text style={styles.columnHeader}>Admission Date</Text>
              ...
              <View key={index} style={styles.tableRow}>
                {selectedData === 'patients' ? (
                  <>
                    ...
                    <Text style={styles.tableCell}>{item.approvalTimestamp}</Text>
                    <Text style={styles.tableCell}>{item.rejectionTimestamp}</Text>
                    <Text style={styles.tableCell}>{item.rating}</Text>
                    <Text style={styles.tableCell}>{translateProfessionalismScores(item.professionalismScores)}</Text>
                  </>
                ) : (
                  <>
                    <Text style={styles.tableCell}>{item.admissionDate}</Text>
                    ...
                    <Text style={styles.tableCell}>{item.createBy_id}</Text>
                    <Text style={styles.tableCell}>{item.hn}</Text>
                    <Text style={styles.tableCell}>{item.procedureLevel}</Text>
                    <Text style={styles.tableCell}>{item.procedureType}</Text>
                    <Text style={styles.tableCell}>{item.approvedByName}</Text>
